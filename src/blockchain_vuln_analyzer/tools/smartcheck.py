...
SMARTCHECK_TOOL_DEFINITION = types.Tool(
    name="smartcheck-analyze",
    description="Analyze Solidity smart contracts for vulnerabilities using SmartCheck static analysis.",
    inputSchema={
        "type": "object",
        "properties": {
            "contract_code": {
            ...
            "rules_file": {
                "type": "string",
                "description": "Path to a custom SmartCheck rules XML file (optional)"
            },
            "output_format": {
                "type": "string",
                "enum": ["text", "json"],
...
async def run_smartcheck_analysis(
    contract_code: str = None,
    contract_file: str = None,
    rules_file: str = None,
    output_format: str = "text"
) -> dict:
    """Run SmartCheck static analysis on a smart contract."""
    try:
        temp_file = None
        contract_path = None
        if contract_code:
        ...
        # Build SmartCheck command
        cmd = ["smartcheck", "-p", contract_path]
        if rules_file:
        ...
        # SmartCheck may print warnings to stderr but still succeed
        if process.returncode != 0:
            # If output is empty, treat as error
            if not output.strip():
                return {
                    "success": False,
                    "error": f"SmartCheck analysis failed: {error_output.strip()}",
                    "tool": "smartcheck"
                }
        ...
        return {
            "success": False,
            "error": f"SmartCheck analysis error: {str(e)}",
            "tool": "smartcheck"
        }

def format_smartcheck_response(result: dict, analysis_id: str) -> str:
    if result["success"]:
        response_text = f"SmartCheck analysis completed.\n\nAnalysis ID: {analysis_id}\n"
        ...
        if output:
            ...
            response_text += "\nNo output from SmartCheck."
    else:
        response_text = f"SmartCheck analysis failed: {result['error']}"
